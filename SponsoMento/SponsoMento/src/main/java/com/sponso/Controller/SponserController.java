...
public class SponserController {
	...
	@SuppressWarnings({ "rawtypes" })
	@CrossOrigin
	@RequestMapping(value = "/sponsor/mentor", method = RequestMethod.POST, produces = "application/json")
	@ApiOperation(value = "Getting Sponser", notes = "")
    @ApiResponses(value = { @ApiResponse(code = 201, message = "valid user"),
    @ApiResponse(code = 404, message = "un-authorized user") })
	@ResponseBody
	public ResponseEntity mentor(@RequestBody MentorLink link){
		
		
		if(sponservice.mentor(link)) 
		return ResponseEntity.status(200).body("{\"success\":\"Menting done\"}");
		else
			return ResponseEntity.status(400).body("{\"error\":\"No Data Found\"}");
	}
	...
	@SuppressWarnings({ "rawtypes" })
	@CrossOrigin
	@RequestMapping(value = "/sponsor/mentored/{id}", method = RequestMethod.GET, produces = "application/json")
	@ApiOperation(value = "Getting Sponser", notes = "")
    @ApiResponses(value = { @ApiResponse(code = 201, message = "valid user"),
    @ApiResponse(code = 404, message = "un-authorized user") })
	@ResponseBody
	public ResponseEntity getmentored(@RequestParam("id")int id){
		
		List<MentorLink> list = sponservice.getMentored(id);
		List<Student> st= new ArrayList<Student>();
		for(int i = 0;i<list.size();i++){
			Student s = new Student();
			s=stservice.getbyid(list.get(i).getStudentId());
